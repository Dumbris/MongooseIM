...
%%% @copyright (C) 2014, Algis Dumbris a.dumbris@gmail.com
%%% @doc
%%% Listener process spawned for internal cloud user. Based on original ejabberd_c2s.erl listener.
%%% Auth, encription and features request removed.
%%% @end
%%% Created : 2014-04-19 13:53:12.416680
%%%-------------------------------------------------------------------

-module(ejabberd_internal2s).
-author('a.dumbris@gmail.com').
...
step_wait_for_session_or_sm(StateData0) ->
...
-spec configure_client(Item :: ejabberd:xml_stream_item(),
    State :: state()) -> fsm_return().
configure_client({xmlstreamstart, _Name, Attrs}, StateData0) ->
    %get U from get_uath_tags()
    U = <<"user">>,
    Server = jlib:nameprep(xml:get_attr_s(<<"to">>, Attrs)),
    try
        true = lists:member(Server, ?MYHOSTS),
        change_shaper(StateData0, jlib:make_jid(<<>>, Server, <<>>)),
        StateData1 = StateData0#state{ streamid = new_id(), authenticated = true, auth_module = 'ac_internal', user = U, lang = get_lang()},
        StateData2 = step_wait_for_auth(StateData1),
        allow = acl:match_rule(StateData2#state.server, StateData2#state.access, StateData2#state.jid),
        StateData3 = step_wait_for_session_or_sm(StateData2),
        %Res = jlib:make_result_iq_reply(El),
        %send_element(StateData3, Res),
        change_shaper(StateData3, JID),
        fsm_next_state(session_established, StateData3)
    catch
        _Error:_Pattern ->
            send_header(StateData0, ?MYNAME, "", get_lang()),
            send_element(StateData0, ?HOST_UNKNOWN_ERR),
            send_trailer(StateData0),
            {stop, normal, StateData0}
    end;

configure_client(timeout, StateData) ->
    {stop, normal, StateData};
configure_client({xmlstreamelement, _}, StateData) ->
    send_element(StateData, ?INVALID_XML_ERR),
    send_trailer(StateData),
    {stop, normal, StateData};
configure_client({xmlstreamend, _}, StateData) ->
    send_element(StateData, ?INVALID_XML_ERR),
    send_trailer(StateData),
    {stop, normal, StateData};
configure_client({xmlstreamerror, _}, StateData) ->
    send_header(StateData, ?MYNAME, "1.0", ""),
    send_element(StateData, ?INVALID_XML_ERR),
    send_trailer(StateData),
    {stop, normal, StateData};
configure_client(closed, StateData) ->
    {stop, normal, StateData}.
